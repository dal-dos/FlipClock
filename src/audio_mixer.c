...
void AudioMixer_readWaveFileIntoMemory(char *fileName, wavedata_t *pSound)
{
	...
	fclose(file); // Should this be here?
}
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	if (!success) {
		printf("Error: sound bite queue is full.\n");
	}
	}
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	for (int i = 0; i < MAX_SOUND_BITES; i++) {
		if (soundBites[i].pSound != NULL) {
			...
			for (int k = 0; k < size && offset < soundBites[i].pSound->numSamples; k++) {
				int sum = (int)buff[k] + (int)soundBites[i].pSound->pData[offset];
				if (sum > SHRT_MAX) { buff[k] = SHRT_MAX; }
				else if (sum < SHRT_MIN) { buff[k] = SHRT_MIN; }
				else { buff[k] = (short)sum; }
				offset++;
			}
